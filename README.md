...
## Замечание:
> Примеры из учебника рассмотрены на языке perl или в программе grep которая имеется только в системах Unix
> но ее нет на Windows! Большенство примеров я не смог повторить, в основном примеры из этого документа
> составил я основываясь на общих возможностей языка регулярных выражений используя python.
...
> __Жадный поиск__: Обычный квантификатор изначально является "жадным". Жадный квантификатор в первую очередь стремится обнаружить 
...
> __Сверхжадный поиск__: пытается найти совпадение, захватывая целиком всю строку, но делает это в рамках одной попытки,
...
> Символьные классы POSIX не поддерживаются в Python. Вместо них используются обычные символьные классы вроде \d или \w
